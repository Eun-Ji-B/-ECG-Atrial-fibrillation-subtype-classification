...
##################
# TFlite 모델 추론 (Uint8 / int8)
#from tflite_runtime.interpreter import Interpreter
from tflite_runtime.interpreter import Interpreter

MODEL_PATH = "./Uint8_model1127.tflite"
...
def run_inference_on_npy(cqt_2d):
    """
    cqt_2d : compute_cqt_mag(seg_10, 200)에서 나온 2D CQT (예: (H, W))
    TFLite 모델 입력: [1, H, W, 3] → int8/uint8로 양자화해서 넣기
    """
    ...
    input_data = input_data.astype(np.float32)    # 양자화 전에 FP32

    # 4-1) quantization 정보 가져와서 int8로 변환
    scale, zero_point = input_details[0]["quantization"]  # (scale, zero_point)
    if scale == 0:
        raise ValueError("입력 텐서 quantization scale 이 0입니다. 모델 확인이 필요합니다.")

    input_data_q = np.round(input_data / scale + zero_point)
    input_data_q = np.clip(input_data_q, -128, 127).astype(np.int8)

    # 5) TFLite에 입력 세팅 + 추론
    interpreter.set_tensor(input_details[0]['index'], input_data_q)
    proc = psutil.Process(os.getpid())

    # ★ 추가: CPU 사용률 초기화 (invoke 구간 평균을 얻기 위해)
...
def main():
    ...
    _ = proc_total.cpu_percent(interval=None)
    ...
    total_cpu_percent_all = proc_total.cpu_percent(interval=None)
    ...
    # ★ 추가: 처리량(Throughput), 평균 invoke 시간, 평균 CPU 사용률 (invoke 구간 기준)
    ...
    if len(results) > 0:
        try:
            ...
            df_res.to_csv("raspi_tflite_results_uint8.csv", index=False, encoding="utf-8-sig")
            print("\n결과가 'raspi_tflite_results_uint8.csv' 파일로 저장되었습니다.")
        except Exception as e:
    ...
    # 맨 마지막에 한 번 더 요약해서 보고 싶을 때 (마지막 invoke 기준)
    print("\n마지막 invoke 기준 상세 로그:")
